...
function log(x: any, y?: any, z?: any) {
...
var activeParamPath = "";
var activeParam: LiveAPIObject;

var trackListener = new LiveAPI(function (v: any[]) {
  // log("trackListener " + v);
...
var deviceListener = new LiveAPI(function (v: any[]) {
...
var activeScenes: [Scene, Scene] = [scenes[0], scenes[1]];
var targetMappingScene: Scene = scenes[1];

function getLockedParameterByPath(path: string, scene: Scene) {
  for (var i = 0; i < scene.lockedParameters.length; i++) {
    var lockedParameter = scene.lockedParameters[i];
    if (lockedParameter.path === path) return lockedParameter;
  }

  return undefined;
}

function createLockedParameter(path: string, scene: Scene) {
  var lockedParameter: LockedParameter = { path: path, targetValue: 0 };
  scene.lockedParameters.push(lockedParameter);
  return lockedParameter;
}

function getOrCreateLockedParameterByPath(path: string, scene: Scene) {
  var param = getLockedParameterByPath(path, scene);

  if (param) return param;
  else return createLockedParameter(path, scene);
}
...
interface TrackedParameter {
  lastValue: number;
  liveApiObject: LiveAPIObject;
}

var trackedParameters = {} as Record<string, TrackedParameter>;

function isParameterTracked(path: string) {
...
var updateTask = new Task(function () {
  if (crossfaderTarget === lastCrossfaderTarget) return;
...
function handleActiveParamValueChange(newValue: number) {
  ...
  var param = getOrCreateLockedParameterByPath(
    activeParamPath,
    targetMappingScene
  );
...
function handleSceneButton(buttonIndex: number, value: number) {
...
var activeParametersForFader = {} as Record<
  string,
  { targetValuesBySide: [number, number] }
>;
var activeParametersForFaderKeys = [] as string[];
...
function updateActiveParametersForFader() {
  ...
  for (var sceneIndex = 0; sceneIndex < 2; sceneIndex++) {
    ...
    for (
      var paramIndex = 0;
      paramIndex < activeScenes[sceneIndex].lockedParameters.length;
      paramIndex++
    ) {
...
function handleCrossfader(amount: number) {
  crossfading = true;

  // var start = Date.now();
  for (
    var keyIndex = 0;
    keyIndex < activeParametersForFaderKeys.length;
    keyIndex++
  ) {
    // var start = Date.now();
